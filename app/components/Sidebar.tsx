  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <>
            {/* Backdrop (right side, closes menu) */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {/* Nav panel — slides in like Piranha Bar */}
            <motion.nav
              ...
              style={{
                ...
                flexDirection: "column",
                justifyContent: "space-between",
                padding: "28px 0 48px 70px",
                overflow: "hidden",
              }}
            >
              {/* X close button — top left of panel, like Piranha Bar */}
              <button
                onClick={() => setOpen(false)}
                style={{
                  background: "none",
              ...
              {/* Nav links — bold mains, lighter sub-items */}
              <ul style={{ listStyle: "none", lineHeight: 0.85, flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", gap: 0 }}>
                {/* HOME */}
                <NavItem label="HOME" onClick={() => { setOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); }} bold delay={0.05} />
...
function NavItem({ label, onClick, bold, delay }: { label: string; onClick: () => void; bold?: boolean; delay: number }) {
  return (
    <motion.li
...
function SubItem({ label, onClick, delay }: { label: string; onClick: () => void; delay: number }) {
  return (
    <motion.li
