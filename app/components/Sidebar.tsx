...
import type { SiteSettings, NavItem as NavItemData } from "@/sanity/lib/queries";
...
const DEFAULT_NAV: NavItemData[] = [
  { _key: "home", label: "HOME", link: "/", emphasis: true },
  { _key: "work", label: "WORK", link: "/work", emphasis: true },
  { _key: "studio", label: "Studio", link: "/work?category=studio" },
  { _key: "vfx", label: "VFX", link: "/work?category=vfx" },
  { _key: "broadcast", label: "Broadcast", link: "/work?category=broadcast" },
  { _key: "originals", label: "Originals", link: "/work?category=originals" },
  { _key: "about", label: "ABOUT", link: "/about", emphasis: true },
];
      ...
      >
        ...
        {!isMobile && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, order: 3 }}>
            {(socials ?? [
              { _key: "li", platform: "linkedin" as const, url: "#" },
              { _key: "ig", platform: "instagram" as const, url: "#" },
              { _key: "vi", platform: "vimeo" as const, url: "#" },
            ]).map((s) => (
              <a
                key={s._key}
                href={s.url}
              ...
              >
                {SOCIAL_ICONS[s.platform] ?? SOCIAL_ICONS.linkedin}
              </a>
            ))}
          </div>
        )}
        ...
        {open && (
          <>
            ...
            >
              ...
              >
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#000000", marginBottom: 6 }}>
                    {contact?.dropInHeading ?? "Drop in:"}
                  </p>
                  <p style={{ fontSize: 12, color: "rgba(0,0,0,0.6)", lineHeight: 1.7, whiteSpace: "pre-line" }}>
                    {contact?.address ?? "Los Angeles, CA"}
                  </p>
                </div>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#000000", marginBottom: 6 }}>
                    {contact?.getInTouchHeading ?? "Get in touch:"}
                  </p>
                  <a
                    href={`tel:${(contact?.phone ?? "+1 (310) 555-1212").replace(/[^+\d]/g, "")}`}
                    style={{ display: "block", fontSize: 12, color: "rgba(0,0,0,0.6)", textDecoration: "none", lineHeight: 1.7 }}
                  >
                    {contact?.phone ?? "+1 (310) 555-1212"}
                  </a>
                  <a
                    href={`mailto:${contact?.email ?? "hello@gtgstudios.com"}`}
                    style={{ display: "block", fontSize: 12, color: "rgba(0,0,0,0.6)", textDecoration: "none" }}
                  >
                    {contact?.email ?? "hello@gtgstudios.com"}
                  </a>
                </div>
              </motion.div>
            </motion.nav>
